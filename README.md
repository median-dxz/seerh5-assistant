...
在packages/app下编写客户端和模组。运行客户端请直接使用`npm run start`，此命令会启动webpack的DevServer，同时在该server内安装最基本的代理中间件。然后在本地服务器上即可使用登陆器游玩。
...
因为这是一个flash的关卡，h5端没有上线，故我们需要在flash端进行抓包和该关卡的as3反编译，最终得到这些参数和发包需要的所有参数。这部分内容在之后的教程中会详细介绍，这边就省略相关过程。
...
因为涉及外部的收发包的抓取，同时需要反查关卡逻辑，所以这部分就是写pve关卡最麻烦的部分。
...
这三个操作其实都是发包操作，但是由于所有的发包命令都统一使用`sendByQueue`封装，所以不用担心乱序的问题。当然，因为这些都是发包操作，所有返回的都是Promise。这里按常理来说，根据实现的不同，我们需要清楚的了解Promise的resolve节点，并且可能需要await和延迟操作来防止不正确的发包顺序，过快发包等操作导致的断线问题，但是如果你使用`SAEngine`模块下的操作，一般除非特别注明，否则resolve的节点都是收包之后，可以放心的顺序使用即可。

下一个操作是`switchBag`，即切换背包，这是一个预置的功能，封装了多组收发包操作，同样也能保证最后一个收包完成才resolve。

因此我们想让以上四个一次性发出，然后等待全部resolve后进行接下来的操作，一般来说应该用`Promise.all`实现。但是由于切换背包的发包和上面不冲突且时间一定远大于上面的操作，故只要在切背包的时候await即可。
...
底层上，`sa-core`已经封装了一组对应的查询api，这里就不多做介绍了。登陆器会使用这些查询函数提供一个查询面板方便开发者编写。你也可以使用浏览器控制台使用这些查询api进行查询，又或者，可以使用第三方的数据源进行查询（如mw手册等）。
...
然后就到了我们最激动人心的自动战斗的设置部分。一般来说，首先不要忘了在脚本运行结束的时候加上：

```js
SABattle.Manager.clear();
```

这里使用了`SABattle`模块的`Manager`导出对象，该对象负责管理自动战斗。`clear`方法会帮我们复位自动战斗的状态，以便切换到其他模型或者正常游戏，战斗结束的时候是**不会**自动复位的，况且有可能因为各种意外情况导致战斗失败或者没有正常结束，此时都需要进行手动的复位。
...
但是对于这个例子，我们使用的模型是：送潘朵+压血魔钰一招秒，事实上，大部分关卡的手打就是这么简单，每个精灵出同一招，要么是几招来回按，切换的顺序也是固定的。那么对于这种送自爆强攻的情况，我们不需要手动编写代码，只需要声明死切链和出招表，就可以生成模型：
...
```
`FightManager`是客户端原生的接口，调用后就会自动进入战斗，主要**不要**在入口函数里写和进入战斗**无关的初始化操作**，入口函数应该尽可能保持简单。
...
- [x] 0.2.x 版本将整体重构为**TypeScript**, 并进行代码结构和逻辑优化
- [x] 0.3.x 版本将进行初步的 UI 设计（暂不支持配置读写保存相关内容）
- [x] 0.4.x 版本将维护一个数据源组件和一个对关键数据进行双向绑定的init模块，且重构之前的面板，使之获取与更新数据的编写更为简洁优雅，同时维护存于cache中的配置
- [ ] --> 0.5.x 版本将使项目具有一定可用性，可以在生产环境使用（不考虑动态加载模组功能），添加单元测试 (on progress)
