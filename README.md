...
手贱操作了一下，文档和以前写的远古脚本全丢了...到时候只能从很老的commit里翻了
...
最后请检查你的后端配置文件，位置是`packages/app/backend/data/data.json`

在其中添加你的相关配置。

**注意目前（app: v0.4.x, v0.5.x，230609版本）的项目内仍存在一些硬编码数据**，因此需要手动更改这些数据适配你的账号才能正常运行，例如日任的配置，sign模组的配置，精灵因子模组的配置等。

上面的问题将会在近期的更新中解决，初步方案是将用户配置全部迁移到后端，用户在后端配置战斗模型，战斗策略，精灵名称和ct的映射，模组配置，持久化的localStorage内容，模组特定配置，高级配置（如cmd监听），app设置等。
