...
const showPveResourcePanel = async (itemValue: string) => {
    ...
    const curModule =
        SeerModuleHelper.currentModule<pveEnterPanel.PveEnterPanel>();
    const panelName = "pveEnterPanel.PveResourceCollection";
    curModule.service.openPanel(panelName);
    await delay(1500);
    const curPanel = curModule.panelMap[
        panelName
    ] as pveEnterPanel.PveResourceCollection;
