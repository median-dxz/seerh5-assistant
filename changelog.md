...
1. 新增`Pet.from`转换`PetLike`对象
   - 构造函数现在只支持从标准`PetInfo`对象创建，这样可以支持更好的typing
   - 从用例上，绝大部分情况都是从`PetInfo`对象创建的
2. 适配新版后端
   - 自动战斗的修改到`battle`模块的`internal`下
   - 新增对音乐加载的修改
   - 精简测试环境
...
1. 迁移入口内容到后端
2. 入口文件更新，现在第一阶段的加载时机是core的脚本载入，但是还未进行初始化
3. 调整资源文件标头以命中缓存
