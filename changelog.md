...
- [x] React中获取图片资源问题的再修复
- [x] 精灵头像显示（简版实现）
- [x] 技能石合成模组
  - [x] 输出使用的石头和概率在控制台
  - [x] `craftOne`指令可以自动选取石头合成一次，d级会留两颗
  - [ ] 不支持显示合成失败
- [x] 梦幻宝石快速合成
  - [x] 位于`sign`模组内，需要参数
- [ ] 因子关卡通用信息获取
...
- [x] 泰坦矿洞切背包bag修复
- [ ] 保存背包bug修复
- [x] 优化`BattlePanel`相关hooks，
  - [x] 将`completed`分离为两个节点，`completed`节点已废弃
  - [x] 新的节点`endPropShown`: 结束面板弹出，并且不会关闭巅峰对战的面板
  - [x] 新的节点`battleEnd`: 战斗结束，节点极其靠后
  - [x] 调整了`roundEnd`节点，同样更加靠后以修复以前节点在对方切换精灵之前导致的bug
