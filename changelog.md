...
- [ ] 快捷命令组模块
  - [ ] 自定义编辑代码并eval
  - [ ] 模块CloseAll
  - [ ] 挂机模式/恢复（避免长时间挂机而由于淘米内存泄漏导致的内存溢出使网页崩溃的问题，方案是关闭所有module后将主面板hide）
- [ ] 在缓存中读取部分配置
...
- [ ] 压血不改变默认首发(压血后还原默认首发)
- [ ] hmr后战斗信息面板同步失效
- [ ] hmr event-handler event hooks失效（结束面板不自动关闭）改进EventTarget，增加事件注册信息保存与获取，方便进行调试分析
- [ ] 校正剩余电池时间
- [ ] bubble增强，使之可以显示html而不是只有纯文本
...
- [x] `tsconfig.json`中`skipLibCheck`选项目前启用
- [x] `const`模块简化
  - multi部分由需要用到的组件/模块自己维护
  - 精灵王id由精灵王日任模块维护
- [x] 精灵王日任
- [x] 技能新技能面板导致的switchBag error(结束自动关闭新技能面板)
- [x] 模组的方法反射功能支持获取参数数量和参数名称
- [x] 取消静态模式，高速进行pve
- [x] ~~签到可以领取活跃度奖励~~该收发包行为暂不支持在h5上执行
- [x] vip点数自动兑换
- [x] 战队强化一键满级
- [x] 静态pve取消双方出招间隔延时，进一步提速
- [x] 友谊之星兑换加入背包页面
- [x] 打开背包和友谊之星自动关闭面板
- [x] 取消磕pp药延迟
- [ ] 日常副本命名重构：realm，更符合业内命名
- [ ] 重构PetBag面板
  - [ ] 抽象并移动弹出菜单
  - [ ] 精灵头像获取
...
v0.3.5.1
1. 将6s对战延迟的计时节点启动战斗函数运行前改为进入战斗首回合后，进一步降低被断线的可能
2. 修复精灵王日任领取周奖励失败的bug
3. 修复精灵王日任领取周奖励不能正确显示完成状态的bug
4. 修复x日任战斗失败导致重复开关关卡的bug
