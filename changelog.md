...
1. 快捷命令组模块(只能说完全是应付一下)
  ...
  - MainPanel.show
1. 在背包页面，可以直接切换动画模式
2. `petBag`中catchTime现在直接复制到剪切板而不是输出到控制台
3. 重构战斗管理器模块和react项目组织，主要变动如下:
...
1. 统一表格组件
2. 修复三塔日任停止后仍然会进行这本层对战的问题
3. ~~压血不改变默认首发(压血后还原默认首发)~~ 请客户端考虑使用更换背包相关功能自行实现
4. ~~hmr后战斗信息面板同步失效~~ 0.4.x版本进行ui和core项目分离后进行解决
5. sa库初始化部分的副作用处理与修复，使之对hmr有更好的支持
