...
class hlak {
    ...
    async runOnce() {
        ...
        await new Promise<void>((resolve, reject) => {
            Functions.LowerBlood(curMod.lowerbloodPets, Const.ITEMS.Potion.中级体力药剂, async () => {
        ...
        BattleModuleManager.queuedModule({
            ...
            async skillModule(info, skills, pets) {
                if (info.isDiedSwitch) {
                    let next = curMod.diedLink.match(pets, info.self.catchtime);
                    if (next != -1) {
