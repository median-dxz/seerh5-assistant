...
- [ ] 完整可读的api doc
- [x] 战斗时隐藏按钮
  - [ ] 可配置可扩展的监听socket监听机制，方便编写状态订阅与同步(应用层)
  - [ ] 订阅背包同步EgretUI状态
  - [ ] 订阅自动治疗同步ReactUI状态
- [ ] 完全分离个人配置，后端集中存储
...
log log log！
魔钰不是在派遣排除列表？
官方自带了一个module显示的log，非常丑，要加入log屏蔽豪华套餐，不过使用的hooks可能可以更新一下
...
- [ ] 因子目前存在稳定的断线情况, 具体来说就是隔一段时间或者场数断一次, 不频繁, 但是怀疑和单纯的两次战斗之间的延迟已经无关联
  ...
  - 貌似是一定时间内32场战斗？
- [ ] 背包失去同步, 内容显示错乱, 怀疑petManager的内部map数据错误，疑似复现办法：压血或者换背包更新背包，概率扰乱PetManager内部map，使用同步背包面板的mod进行ui操作也会弄乱
  - [ ] 先验证官方乐观ui是否会出问题
  - [ ] 尝试对背包更新加锁
  - [ ] 做一遍日任后无法进入因子战斗（预处理部分卡住）
  - [ ] 背包扰乱导致精灵派遣也失败
  - [ ] 都疑似是PetManger的问题
...
1. 修复表格的unique key问题
2. 使用swr库修复不正确的自动治疗切换
3. 修复抓包界面的性能问题

v0.5.0

1. app项目布局全部重构, 前后端部分分离
2. 界面重新设计调整
3. 样式统一化
4. 重构样式组件部分
5. 重构表格
