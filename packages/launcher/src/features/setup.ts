import { EventSource, Hook } from 'sea-core';
...
export function setupForLauncher() {
    ...
    EventSource.hook(Hook.Battle.battleStart).on(resolveStrategy);
    EventSource.hook(Hook.Battle.roundEnd).on(resolveStrategy);
}
